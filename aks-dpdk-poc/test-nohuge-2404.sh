#!/bin/bash
# MANA DPDK test on Ubuntu 24.04 / kernel 6.8 — uses --no-huge to bypass cgroup issue
set -e

echo "[1] HugePages (for later): allocating..."
...
echo "[3] ibv_devices check..."
ibv_devices 2>/dev/null || echo "ibv_devices not in container, checking host..."
...
echo "[5] Binding eth1 to uio_hv_generic..."
