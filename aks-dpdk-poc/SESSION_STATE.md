...
9. **CURRENT BLOCKER: mana0 dev_start fails with -22**: `rte_eth_dev_start()` → `mana_dev_start()` → `mana_start_tx_queues()` → `ibv_create_cq()` returns EINVAL. VPP shows `rx: queues 0, tx: queues 0` suggesting `rte_eth_dev_configure()` completed but queues weren't properly set up. testpmd auto-start works, so the DPDK/ibverbs layer is functional — the issue is in VPP's device setup sequence.
